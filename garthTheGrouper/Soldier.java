					...
					if(rc.canAttackSquare(enemyToAttack.location)) {
						...
						if(fightAlliedHealth < totalEnemyHP) {
							...
							if(alliedHealth > totalEnemyHP && rc.getHealth() > 50 && rc.canMove(retreatDir)){
								...
								rc.setIndicatorString(0, "Suiciding");
								// Suicide if outnumbered and alone
								rc.selfDestruct();
							} else {
								rc.setIndicatorString(0, "Moving to Suicide");
...
//							else{
								// Attack the enemy
								rc.setIndicatorString(0, "Simply attacking");
...
//							}
					...
					rc.setIndicatorString(0, "NOT ACTIVE");
				}
			}
		}
	}
	...
	public static boolean isFormingSquad(RobotController rc, int band) throws GameActionException {
		return rc.readBroadcast(Comm.RESPONDING_SOLDIER_CHANNEL) == band;
	}
