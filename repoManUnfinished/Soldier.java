package repoMan;

import java.util.Random;

import battlecode.common.Clock;
import battlecode.common.Direction;
import battlecode.common.GameActionException;
import battlecode.common.MapLocation;
import battlecode.common.Robot;
import battlecode.common.RobotController;
import battlecode.common.RobotInfo;
import battlecode.common.RobotType;
import battlecode.common.Team;
import battlecode.common.TerrainTile;

public class Soldier{
	public static enum herdingStatus {towards, away, init}
	public static herdingStatus currentHerdingStatus;


	//TODO: later on change from random to prefer squares with high cow content. (or herd to one location and stand on it.)
	public static void wander(RobotController rc, Random random) throws GameActionException {
		if(rc.isActive()) {
			//tell soldiers to start following leader.
			int band = (RobotPlayer.tunedChannel/100)*100;
			rc.broadcast(band + Comm.LOCATION_SUBCHANNEL, -1);

			Direction chosenDirection = Direction.values()[random.nextInt(8)];

			double cows = rc.senseCowsAtLocation(rc.getLocation());
			float prob = (float) Math.exp(-cows/100.0);
			rc.setIndicatorString(2, "Cows are: " + cows + " Prob of moving is: " + prob );
			if(random.nextFloat() < prob && canMove(rc, chosenDirection, false)) {
				rc.sneak(chosenDirection);
			}
		}
	}

	public static boolean canMove(RobotController rc, Direction dir, boolean bugAroundEnemies){
		boolean goAhead = true;
		//If we're bugging around Enemies, check to make sure that we're okay in that direction.
		if(bugAroundEnemies){
			MapLocation trialLoc = rc.getLocation().add(dir);
			for(int i=0; i<RobotPlayer.nearbyEnemiesInfo.length; i++){
				if(RobotPlayer.nearbyEnemiesInfo[i].type == RobotType.SOLDIER && trialLoc.distanceSquaredTo(RobotPlayer.nearbyEnemiesInfo[i].location) <= RobotType.SOLDIER.attackRadiusMaxSquared){
					goAhead = false;
					break;
				}
			}
		}
		return (goAhead && rc.canMove(dir) && rc.getLocation().add(dir).distanceSquaredTo(rc.senseEnemyHQLocation()) > RobotType.HQ.attackRadiusMaxSquared);
	}

	public static boolean nearEnemies(RobotController rc) throws GameActionException{
		Robot[] enemies =  rc.senseNearbyGameObjects(Robot.class, rc.getType().sensorRadiusSquared, rc.getTeam().opponent());
		RobotInfo[] enemiesInfo = new RobotInfo[enemies.length];
		for(int i=0; i < enemies.length; i++){
			enemiesInfo[i] = rc.senseRobotInfo(enemies[i]);

		}
		//Set squad size if it's too big;
		int currentSquadSize = rc.readBroadcast(Comm.SQUAD_SIZE_CHANNEL);
		int maxSquadSize = (2000 - Clock.getRoundNum())/100;

		if(enemies.length > currentSquadSize && enemies.length < maxSquadSize){
			//If you see more enemies around than squad size, then increase squad size to that number.
			rc.broadcast(Comm.SQUAD_SIZE_CHANNEL, enemies.length);
		}
		if(maxSquadSize >= 5 && rc.readBroadcast(Comm.SQUAD_SIZE_CHANNEL) > maxSquadSize){
			//Want to always limit squad size to max squad size.
			rc.broadcast(Comm.SQUAD_SIZE_CHANNEL, maxSquadSize);
		}

		//Save this info to use in other functions later.
		RobotPlayer.nearbyEnemiesInfo = enemiesInfo;
		return enemies.length > 0;
	}
	public static void combatStrategy(RobotController rc, int band, Random random) throws GameActionException {

		RobotInfo[] enemyRobots = RobotPlayer.nearbyEnemiesInfo;

		RobotInfo closestEnemyInfo = null;
		RobotInfo lowestEnemyInfo = null;
		double closestEnemyDistance = 10000;
		double lowestHealth = 100000;

		double totalEnemyHP = 0;
		double fightEnemyHP = 0;
		
		// Find enemy with lowest HP and closest distance

		for(RobotInfo enemyInfo : enemyRobots) {
			int distanceToEnemy = rc.getLocation().distanceSquaredTo(enemyInfo.location);

			if(enemyInfo.type != RobotType.HQ && distanceToEnemy < closestEnemyDistance) {
				closestEnemyDistance = distanceToEnemy;
				closestEnemyInfo = enemyInfo;
				if(enemyInfo.type == RobotType.SOLDIER){
					//only consider soldiers for suicide move.
					totalEnemyHP += enemyInfo.health;
					if(enemyInfo.location.distanceSquaredTo(rc.getLocation()) <= rc.getType().attackRadiusMaxSquared){
						fightEnemyHP += enemyInfo.health;
					}
				}
			}
			//Don't care much for pastrs here. Only need to take care of soldiers.
			if(enemyInfo.type == RobotType.SOLDIER && enemyInfo.health < lowestHealth){
				lowestHealth = enemyInfo.health;
				lowestEnemyInfo = enemyInfo;
			}

		}

		//figure out which enemy to attack (Closest, or, LowestHitPoint);
		RobotInfo enemyToAttack = null;

		//TODO: perhaps better heuristics are needed.
		//perhaps if lowestHitPoint enemy is in attackRange, shoot at him. Or if all I need to do is land one hit, give it a shot.
		if(lowestEnemyInfo != null && (rc.getLocation().distanceSquaredTo(lowestEnemyInfo.location) <= rc.getType().attackRadiusMaxSquared || lowestEnemyInfo.health <= 10)){
			enemyToAttack = lowestEnemyInfo;
		}else{
			enemyToAttack = closestEnemyInfo;
		}


		if(enemyToAttack != null) {


			if(rc.isActive()) {
	
				Robot[] friends = rc.senseNearbyGameObjects(Robot.class, rc.getType().sensorRadiusSquared, rc.getTeam());
				double alliedHealth = rc.getHealth();
				for(int i=0; i<friends.length; i++){
					RobotInfo friend = rc.senseRobotInfo(friends[i]);
					if(friend.type == RobotType.SOLDIER){
						alliedHealth += friend.health;
					}
				}
				//TODO: Change to if total friendly health is less than total enemy health to avoid unnecessary suicide. 
				if(rc.canAttackSquare(enemyToAttack.location)) {
					//Retreat if out numbered but backup is available.
					if(rc.getHealth() < fightEnemyHP && enemyToAttack.health > 20 && alliedHealth < totalEnemyHP) {
						// Send distress signal
						sendDistressSignal(rc, band);
						Direction retreatDir = rc.getLocation().directionTo(enemyToAttack.location).opposite();
						if(alliedHealth > totalEnemyHP && rc.getHealth() >= 30 && canMove(rc, retreatDir, false)){
							// If you have enough health, and there's hope for you, Retreat in opposite direction.
							rc.move(retreatDir);
						}
						else if(rc.getLocation().distanceSquaredTo(enemyToAttack.location) <= 2) {
							// Suicide if out-numbered and alone
							rc.selfDestruct();
						} else if(rc.canMove(rc.getLocation().directionTo(enemyToAttack.location))){
							//TODO: Perhaps a simple move command will do.
							moveToLocation(rc, enemyToAttack.location, random, false, false);
							//BasicPathing.simplemove(rc, enemyToAttack.location);
						} else {
							rc.attackSquare(enemyToAttack.location);
						}
					} else {
						// Attack the enemy
						rc.attackSquare(enemyToAttack.location);
					}

				} else {
					//TODO: when in combat, perhaps a simple move command will do. To reduce bytecode usage.
					//Move toward enemy (if pastr)
					if(enemyToAttack.type == RobotType.PASTR || enemyToAttack.type == RobotType.NOISETOWER || RobotPlayer.panic){
						//moveToLocation(rc, enemyToAttack.location, random, false, false);
						//a simple move perhaps will do:
						if(rc.canMove(rc.getLocation().directionTo(enemyToAttack.location))){
							rc.move(rc.getLocation().directionTo(enemyToAttack.location));
						}

					}else{
						if(alliedHealth < totalEnemyHP ){
							//Retreat away,
							sendDistressSignal(rc, band);
							Direction retreatDir = rc.getLocation().directionTo(enemyToAttack.location).opposite();
							// If you have enough health, and there's hope for you, Retreat in opposite direction.
							if(rc.canMove(retreatDir)){
								rc.move(retreatDir);
							}
						}
						//							//TODO: If there are pastrs to destroy, bug around enemies. Don't stale mate				
						MapLocation[] pastrs = rc.sensePastrLocations(rc.getTeam().opponent());
						if(pastrs.length == 0){
							pastrs = rc.sensePastrLocations(rc.getTeam());
						}
						if(pastrs.length > 0){
							nonCombatStrategy(rc, band, random, true);
						}
						else{
							MapLocation oneCloser = rc.getLocation().add(rc.getLocation().directionTo(enemyToAttack.location));
							if(oneCloser.distanceSquaredTo(enemyToAttack.location)>enemyToAttack.type.attackRadiusMaxSquared){
								moveToLocation(rc, oneCloser, random, false, false);
							}
						}
					}
				}
			}
		}
	}

	public static void issueMoveToEnemyPastrCommand(RobotController rc, int band, MapLocation location) throws GameActionException {
		int commandChannel = band + Comm.COMMAND_SUBCHANNEL;
		int locationChannel = band + Comm.LOCATION_SUBCHANNEL;

		rc.broadcast(commandChannel, Comm.MOVE_TO_ENEMY_PASTR);
		rc.broadcast(locationChannel, VectorActions.locToInt(location));
	}

	public static void issueMoveToFriendlyPastrCommand(RobotController rc, int band, MapLocation location) throws GameActionException {
		int commandChannel = band + Comm.COMMAND_SUBCHANNEL;
		int locationChannel = band + Comm.LOCATION_SUBCHANNEL;

		rc.broadcast(commandChannel, Comm.MOVE_TO_FRIENDLY_PASTR);
		rc.broadcast(locationChannel, VectorActions.locToInt(location));
	}

	public static void issueMoveToLocationCommand(RobotController rc, int band, MapLocation location) throws GameActionException {
		int commandChannel = band + Comm.COMMAND_SUBCHANNEL;
		int locationChannel = band + Comm.LOCATION_SUBCHANNEL;

		rc.broadcast(commandChannel, Comm.MOVE_TO_LOCATION);
		rc.broadcast(locationChannel, VectorActions.locToInt(location));
	}

	public static void issueStandbyCommand(RobotController rc, int band) throws GameActionException {
		int commandChannel = band + Comm.COMMAND_SUBCHANNEL;
		int locationChannel = band + Comm.LOCATION_SUBCHANNEL;

		rc.broadcast(commandChannel, Comm.STANDBY);
		rc.broadcast(locationChannel, Comm.END_OF_COMMAND);
	}

	public static void issueFollowMeCommand(RobotController rc, int band) throws GameActionException {
		int commandChannel = band + Comm.COMMAND_SUBCHANNEL;
		int locationChannel = band + Comm.LOCATION_SUBCHANNEL;

		if(rc.readBroadcast(commandChannel) != Comm.WANDER) {
			rc.broadcast(commandChannel, Comm.WANDER);
		}

		rc.broadcast(locationChannel, 10100+VectorActions.locToInt(rc.getLocation()));
	}
	
	public static void issueHerdCommand(RobotController rc, int band, MapLocation pastrLocation) throws GameActionException {
		int commandChannel = band + Comm.COMMAND_SUBCHANNEL;
		int locationChannel = band + Comm.LOCATION_SUBCHANNEL;
		
		rc.broadcast(commandChannel, Comm.HERD);
		rc.broadcast(locationChannel, VectorActions.locToInt(pastrLocation));
	}

	public static void moveToLocation(RobotController rc, MapLocation location, Random random, boolean sneak, boolean bugAroundEnemies) throws GameActionException {
		rc.setIndicatorString(1, "Trying to move to " + location);
		//BasicPathing.bug(rc, location, random, sneak, bugAroundEnemies);
		BasicPathing.snail(rc, location, true, sneak, bugAroundEnemies);
	}

	/**
	 * Selects enemy PASTR which is closest to friendly HQ and issues a MoveToEnemyPastr command
	 * @param rc The RobotController of the player calling this function
	 * @param random The Random object associated with this player
	 * @param squadNumber The squad number of the player calling this function
	 * @param pastrs A list of available PASTRs
	 * @throws GameActionException
	 */
	public static void moveToEnemyPastr(RobotController rc, Random random, int squadNumber, MapLocation[] pastrs, boolean bugAroundEnemies) throws GameActionException {
		MapLocation ourHqLocation = rc.senseHQLocation();
		MapLocation closestPastr = null;
		int closestDistance = Integer.MAX_VALUE;
		for(MapLocation pastr : pastrs) {
			int distanceToOurHq = pastr.distanceSquaredTo(ourHqLocation);
			if(distanceToOurHq <= closestDistance) {
				closestDistance = distanceToOurHq;
				closestPastr = pastr;
			}
		}
		issueMoveToEnemyPastrCommand(rc, squadNumber, closestPastr);
		moveToLocation(rc, closestPastr, random, false, bugAroundEnemies);
	}
	
	/**
	 * Selects friendly PASTR which is closest to enemy HQ and issues a MoveToFriendlyPastr command
	 * @param rc The RobotController of the player calling this function
	 * @param random The Random object associated with this player
	 * @param squadNumber The squad number of the player calling this function
	 * @param pastrs A list of available PASTRs
	 * @throws GameActionException
	 */
	public static void moveToFriendlyPastr(RobotController rc, Random random, int squadNumber, MapLocation[] pastrs, boolean bugAroundEnemies) throws GameActionException {
		MapLocation theirHqLocation = rc.senseEnemyHQLocation();
		MapLocation closestPastr = null;
		int closestDistance = Integer.MAX_VALUE;
		for(MapLocation pastr : pastrs) {
			int distanceToTheirHq = pastr.distanceSquaredTo(theirHqLocation);
			if(distanceToTheirHq <= closestDistance) {
				closestDistance = distanceToTheirHq;
				closestPastr = pastr;
			}
		}
		issueMoveToFriendlyPastrCommand(rc, squadNumber, closestPastr);
		moveToLocation(rc, closestPastr, random, false, bugAroundEnemies);
	}

	public static void verifyStandingPastrMove(RobotController rc, int band, Random random, boolean isEnemyPASTR, boolean bugAroundEnemies) throws GameActionException {
		MapLocation target = VectorActions.intToLoc(rc.readBroadcast(band + Comm.LOCATION_SUBCHANNEL));
		if(isEnemyPASTR && isValidPastr(rc, target, rc.getTeam().opponent())) {
			// Continue move command to target
			moveToLocation(rc, target, random, false, bugAroundEnemies);
		} else if(!isEnemyPASTR && isValidPastr(rc, target, rc.getTeam())) {
			if(isAtLocation(rc, target, true)) {
				// Start herding cows
				issueHerdCommand(rc, band, target);
			} else {
				// Continue sneak command to target
				moveToLocation(rc, target, random, true, bugAroundEnemies);
			}
		} else {
			issueStandbyCommand(rc, band);
		}
	}

	public static void verifyStandingMove(RobotController rc, int band, Random random, boolean bugAroundEnemies) throws GameActionException {
		MapLocation target = VectorActions.intToLoc(rc.readBroadcast(band + Comm.LOCATION_SUBCHANNEL));
		if(isAtLocation(rc, target, true)) {
			issueStandbyCommand(rc, band);
		} else {
			// Continue move command to target
			moveToLocation(rc, target, random, false, bugAroundEnemies);
		}
	}
	
	public static void verifyStandingHerd(RobotController rc, int band, Random random, boolean isEnemyPastr, boolean bugAroundEnemies) throws GameActionException {
		MapLocation pastrLocation = VectorActions.intToLoc(rc.readBroadcast(band + Comm.LOCATION_SUBCHANNEL));
		if(!isEnemyPastr && isValidPastr(rc, pastrLocation, rc.getTeam())) {
			moveToLocation(rc, pastrLocation, random, true, bugAroundEnemies);
		} else {
			issueStandbyCommand(rc, band);
		}
	}

	private static boolean isValidPastr(RobotController rc, MapLocation location, Team team) {
		MapLocation[] pastrs = RobotPlayer.friendlyPastrs;
		if(team.equals(rc.getTeam().opponent())){
			pastrs = RobotPlayer.enemyPastrs;
		}
		for(MapLocation pastrLocation : pastrs) {
			if (pastrLocation.equals(location)) {
				return true;
			}
		}
		return false;
	}

	public static void broadcastVitality(RobotController rc, int band) throws GameActionException {
		int vitalityChannel = band + Comm.VITALITY_SUBCHANNEL;
		rc.broadcast(vitalityChannel, Clock.getRoundNum());
	}

	public static boolean isLeaderAlive(RobotController rc, int band) throws GameActionException {
		int vitalityChannel = band + Comm.VITALITY_SUBCHANNEL;
		int roundsSinceLastVitalityBroadcast = Clock.getRoundNum() - rc.readBroadcast(vitalityChannel);
		return roundsSinceLastVitalityBroadcast <= 2;
	}

	public static void sendDistressSignal(RobotController rc, int band) throws GameActionException {
		int distressChannel = band + Comm.DISTRESS_SUBCHANNEL;
		rc.broadcast(distressChannel, VectorActions.locToInt(rc.getLocation()));
	}

	public static boolean isDistressSignal(RobotController rc, int band) throws GameActionException {
		int distressChannel = band + Comm.DISTRESS_SUBCHANNEL;
		return rc.readBroadcast(distressChannel) != 0;
	}

	public static void answerDistressSignal(RobotController rc, int band, Random random, boolean bugAroundEnemies) throws GameActionException {
		//TODO: I currently have set answering distress signal to be aggressive
		int distressChannel = band + Comm.DISTRESS_SUBCHANNEL;
		MapLocation locationOfDistress = VectorActions.intToLoc(rc.readBroadcast(distressChannel));

		// Clear distress signal
		rc.broadcast(distressChannel, 0);

		// Move to location of distress
		issueMoveToLocationCommand(rc, band, locationOfDistress);
		moveToLocation(rc, locationOfDistress, random, false, false);
	}

	public static boolean isHQCommand(RobotController rc, int band) throws GameActionException {
		int HQCommandChannel = band + Comm.HQ_COMMAND_SUBCHANNEL;
		return rc.readBroadcast(HQCommandChannel) != Comm.STANDBY;
	}

	public static void followHQCommand(RobotController rc, int band) throws GameActionException {
		int HQCommandChannel = band + Comm.HQ_COMMAND_SUBCHANNEL;
		int HQCommand = rc.readBroadcast(HQCommandChannel);
		int HQLocationChannel = band + Comm.HQ_LOCATION_SUBCHANNEL;

		if(HQCommand == Comm.ALL_SQUADS_TO_LOCATION) {
			MapLocation target = VectorActions.intToLoc(rc.readBroadcast(HQLocationChannel));

			issueMoveToLocationCommand(rc, band, target);
		}

		// Clear HQ Command
		rc.broadcast(HQCommandChannel, Comm.STANDBY);
		rc.broadcast(HQLocationChannel, 0);
	}

	public static boolean isFormingSquad(RobotController rc, int band) throws GameActionException {
		return rc.readBroadcast(Comm.RESPONDING_SOLDIER_CHANNEL) == band && rc.senseRobotCount() <= rc.readBroadcast(Comm.SQUAD_SIZE_CHANNEL) && !RobotPlayer.panic;
	}

	public static void updateLeaderLocation(RobotController rc, int band) throws GameActionException {
		int commandChannel = band + Comm.COMMAND_SUBCHANNEL;
		int leaderLocationChannel = band + Comm.LEADER_LOCATION_SUBCHANNEL;
		int leaderLocation = VectorActions.locToInt(rc.getLocation());

		int command = rc.readBroadcast(commandChannel);
		if(command == Comm.WANDER || command == Comm.MOVE_TO_FRIENDLY_PASTR) {
			leaderLocation += 10100;
		}

		rc.broadcast(leaderLocationChannel, leaderLocation);

	}

	public static void shouldIpanicOrRush(RobotController rc) throws GameActionException {
		int panic = rc.readBroadcast(Comm.PANIC_CHANNEL);
		if(panic>0){
			RobotPlayer.panic = true;
		}
		int rush = rc.readBroadcast(Comm.RUSH_CHANNEL);
		if(rush>0){
			RobotPlayer.rush = true;
		}
	}
	
	public static boolean isAtLocation(RobotController rc, MapLocation location, boolean nearBy) {
		MapLocation myLocation = rc.getLocation();
		if(nearBy) {
			return myLocation.equals(location) || myLocation.isAdjacentTo(location);
		} else {
			return myLocation.equals(location);
		}
	}
	
	public static void herdCows(RobotController rc, int band, Random random, int herderIndex, boolean bugAroundEnemies) throws GameActionException {
		MapLocation pastrLocation = VectorActions.intToLoc(rc.readBroadcast(band + Comm.LOCATION_SUBCHANNEL));
		if(herderIndex >= 0 && herderIndex <= 7) {
			MapLocation startingLocation = pastrLocation.add(Direction.values()[herderIndex], 3);
			TerrainTile tileAtStart = rc.senseTerrainTile(startingLocation);
			while(tileAtStart == TerrainTile.OFF_MAP || tileAtStart == TerrainTile.VOID) {
				// Move start closer to PASTR
				startingLocation = startingLocation.add(Direction.values()[(herderIndex + 4) % 8]);
				tileAtStart = rc.senseTerrainTile(startingLocation);
			}

			MapLocation awayLocation = startingLocation;
			for(int i = 1; i <= 8; i++) {
				// Try to expand
				MapLocation newAwayLocation = awayLocation.add(Direction.values()[herderIndex]);
				TerrainTile tileAtNewLocation = rc.senseTerrainTile(newAwayLocation);
				if(tileAtNewLocation != TerrainTile.OFF_MAP && tileAtNewLocation != TerrainTile.VOID) {
					awayLocation = newAwayLocation;
				} else {
					break;
				}
			}

			if(currentHerdingStatus == herdingStatus.init) {
				rc.setIndicatorString(1, "Herding init " + herderIndex);
				if(isAtLocation(rc, startingLocation, false)) {
					currentHerdingStatus = herdingStatus.away;
				} else {
					moveToLocation(rc, startingLocation, random, true, bugAroundEnemies);
				}
			} else if(currentHerdingStatus == herdingStatus.away) {
				rc.setIndicatorString(1, "Herding away " + herderIndex);
				if(isAtLocation(rc, awayLocation, false)) {
					currentHerdingStatus = herdingStatus.towards;
				} else {
					moveToLocation(rc, awayLocation, random, true, bugAroundEnemies);
				}
			} else if(currentHerdingStatus == herdingStatus.towards) {
				rc.setIndicatorString(1, "Herding towards " + herderIndex);
				if(isAtLocation(rc, startingLocation, false)) {
					currentHerdingStatus = herdingStatus.away;
				} else {
					moveToLocation(rc, startingLocation, random, false, bugAroundEnemies);
				}
			}
		} else if(herderIndex == 8){
			// Sneak around PASTR
			moveToLocation(rc, pastrLocation, random, true, bugAroundEnemies);
		} else {
			System.out.println("Shouldn't be here");
		}
	}
	
	public static int getHerderIndex(RobotController rc, int band) throws GameActionException {
		int herdingRootChannel = band + Comm.HERDER_ROOT_SUBCHANNEL;
		for(int i = 0; i < 8; i++) {
			if(rc.readBroadcast(herdingRootChannel + i) == 0) {
				rc.broadcast(herdingRootChannel + i, 1);
				currentHerdingStatus = herdingStatus.init;
				return i;
			}
		}
		return 8;
	}

	public static void selectChannel(RobotController rc) throws GameActionException {
		// If standing by, pay attention to Idle Soldier Channel
		if(rc.readBroadcast(Comm.IDLE_SOLDIER_CHANNEL) == Comm.TUNE_IN) {
			RobotPlayer.tunedChannel = Comm.RESPONDING_SOLDIER_CHANNEL;
		}
		//Check if need to build PASTR or NOISETOWER
		int build = rc.readBroadcast(Comm.BUILD_PASTR_CHANNEL);
		if(build > 0){
			rc.setIndicatorString(0, "Yup, build pstr channel is not 0 its: " + build);
			int buildTally = rc.readBroadcast(Comm.BUILD_PASTR_TALLY_CHANNEL);
			if(buildTally%2==1){
				//If tally is even build pastr 
				RobotPlayer.tunedChannel = Comm.GOOD_PASTR_CHANNEL;
				RobotPlayer.myBuildLoc = rc.readBroadcast(Comm.GOOD_PASTR_CHANNEL);
			}
			else{
				//if tally is odd build noise tower
				RobotPlayer.tunedChannel = Comm.GOOD_NOISE_CHANNEL;
				RobotPlayer.myBuildLoc = rc.readBroadcast(Comm.GOOD_NOISE_CHANNEL);
			}
			//claim the work as done.
			rc.broadcast(Comm.BUILD_PASTR_CHANNEL, 0);
			rc.broadcast(Comm.BUILD_PASTR_TALLY_CHANNEL, buildTally+1);
		}	
	}

	public static void selectBand(RobotController rc) throws GameActionException {
		if(rc.readBroadcast(Comm.RESPONDING_SOLDIER_TALLY_CHANNEL) > 0) {
			// If tuned in for command, change channel to specified band
			RobotPlayer.tunedChannel = rc.readBroadcast(Comm.RESPONDING_SOLDIER_CHANNEL);
			rc.broadcast(Comm.RESPONDING_SOLDIER_TALLY_CHANNEL, rc.readBroadcast(Comm.RESPONDING_SOLDIER_TALLY_CHANNEL) - 1);
		} else {
			// Go back to the idle channel
			RobotPlayer.tunedChannel = Comm.IDLE_SOLDIER_CHANNEL;
		}
	}

	public static void goBuildPastr(RobotController rc, Random random, boolean bugAroundEnemies) throws GameActionException {
		int pastrLoc = RobotPlayer.myBuildLoc;
		//System.out.println("I'm moving to build pastr.");
		//Soldier will move to build pastr
		MapLocation loc = VectorActions.intToLoc(pastrLoc);
		moveToLocation(rc, loc, random, false, bugAroundEnemies);

		//If pastr's already built, go to idle.

		if(rc.getLocation().equals(loc)){
			while(!rc.isActive()){
				rc.yield();
			}
			rc.construct(RobotType.PASTR);

		}else if(rc.canSenseSquare(loc) && rc.senseObjectAtLocation(loc) != null){
			//If pastr's already built, go to idle.
			RobotPlayer.tunedChannel = Comm.IDLE_SOLDIER_CHANNEL;
		}
	}

	public static void goBuildNoiseTower(RobotController rc, Random random, boolean bugAroundEnemies) throws GameActionException {
		int noiseLoc = RobotPlayer.myBuildLoc;
		//System.out.println("I'm moving to build noise tower.");
		//Soldier will move to build noisetower next to pastr
		MapLocation loc = VectorActions.intToLoc(noiseLoc);	
		moveToLocation(rc, loc, random, false, bugAroundEnemies);


		if(rc.getLocation().equals(loc)){
			while(!rc.isActive()){
				rc.yield();
			}
			rc.construct(RobotType.NOISETOWER);

		}else if(rc.canSenseSquare(loc) && rc.senseObjectAtLocation(loc) != null){
			//IF tower's already built, go to idle.
			RobotPlayer.tunedChannel = Comm.IDLE_SOLDIER_CHANNEL;
		}	
	}

	public static void defenseStrategy(RobotController rc, int band, Random random, boolean bugAroundEnemies) throws GameActionException {
		// Soldier is tuned into a squad band
		if(Comm.isOnSubchannel(RobotPlayer.tunedChannel, Comm.SIGN_IN_SUBCHANNEL)) {
			if(rc.readBroadcast(RobotPlayer.tunedChannel) == 0) {
				// If this squad has no leader, become the leader
				RobotPlayer.isLeader = true;
				initializeLeader(rc, band);
				rc.broadcast(RobotPlayer.tunedChannel, 1);
				RobotPlayer.tunedChannel = band + Comm.COMMAND_SUBCHANNEL;
				rc.broadcast(RobotPlayer.tunedChannel, Comm.STANDBY);
			} else {
				RobotPlayer.tunedChannel = band + Comm.LEADER_LOCATION_SUBCHANNEL;
			}
		}

		/*
		 * Strategy:
		 * 		If enemy pastures are present, attack them.
		 *      If not, protect our pastrs.
		 *      If we have none, wander around, search for enemies to attack
		 */
		if(RobotPlayer.isLeader) {
			// Leader Strategy

			if(isFormingSquad(rc, band)) {
				// Wander around so that area around HQ is not congested
				wander(rc, random);
				issueFollowMeCommand(rc, band);
			} else if(isHQCommand(rc, band)) {
				rc.setIndicatorString(1, "Following Order from HQ");
				followHQCommand(rc, band);
			} else if(isDistressSignal(rc, band)) {
				rc.setIndicatorString(1, "Answering Distress Signal");
				answerDistressSignal(rc, band, random, bugAroundEnemies);
			} else {
				int outstandingCommand = rc.readBroadcast(RobotPlayer.tunedChannel);
				rc.setIndicatorString(1, "Current Command: " + outstandingCommand);

				MapLocation[] pastrsToMoveTo = RobotPlayer.enemyPastrs;
				boolean isEnemyPASTR = true;
				if(pastrsToMoveTo.length == 0) {
					// If no enemy PASTRs, move to protect our PASTRs
					pastrsToMoveTo = RobotPlayer.friendlyPastrs;
					isEnemyPASTR = false;
				}
				if(pastrsToMoveTo.length > 0) {
					if(outstandingCommand == Comm.STANDBY) {
						// Select new target to move to
						if(isEnemyPASTR) {
							moveToEnemyPastr(rc, random, band, pastrsToMoveTo, bugAroundEnemies);
						} else {
							moveToFriendlyPastr(rc, random, band, pastrsToMoveTo, bugAroundEnemies);
						}
					} else if(outstandingCommand == Comm.MOVE_TO_ENEMY_PASTR) {
						// Verify existing move command
						verifyStandingPastrMove(rc, band, random, isEnemyPASTR, bugAroundEnemies);
					} else if(outstandingCommand == Comm.MOVE_TO_FRIENDLY_PASTR) {
						// Verify existing move command
						verifyStandingPastrMove(rc, band, random, isEnemyPASTR, bugAroundEnemies);
					} else if(outstandingCommand == Comm.MOVE_TO_LOCATION) {
						// Verify if at target
						verifyStandingMove(rc, band, random, bugAroundEnemies);
					} else if(outstandingCommand == Comm.WANDER) {
						// Select new target to move to
						if(isEnemyPASTR) {
							moveToEnemyPastr(rc, random, band, pastrsToMoveTo, bugAroundEnemies);
						} else {
							moveToFriendlyPastr(rc, random, band, pastrsToMoveTo, bugAroundEnemies);
						}
					}else if(outstandingCommand == Comm.HERD) {
						verifyStandingHerd(rc, band, random, isEnemyPASTR, bugAroundEnemies);
					}
				} else {						
					if(outstandingCommand == Comm.STANDBY) {
						// Wander
						wander(rc, random);
						issueFollowMeCommand(rc, band);
					} else if(outstandingCommand == Comm.MOVE_TO_ENEMY_PASTR){
						// Wander
						wander(rc, random);
						issueFollowMeCommand(rc, band);
					} else if(outstandingCommand == Comm.MOVE_TO_LOCATION) {
						// Verify if at target
						verifyStandingMove(rc, band, random, bugAroundEnemies);
					} else if(outstandingCommand == Comm.WANDER) {
						// Wander
						wander(rc, random);
						issueFollowMeCommand(rc, band);
					}
				}	
			}
		} else {
			// Follower strategy = Follow the leader

			//IF you're a follower, and we're rushing, and there's no enemies in sight(already satisfied here), then you have 50% chance of just building a pastr.
			//If you're a follower, and we're panicking, there's a 0.5*0.5 chance of building a pastr.
			double cows = rc.senseCowsAtLocation(rc.getLocation());
			if(cows > 3000 || ((RobotPlayer.rush || (RobotPlayer.panic && random.nextFloat() < 0.5) ) && cows > 100 && random.nextFloat() < 0.5)){
				while(!rc.isActive()){
					rc.yield();
				}
				rc.construct(RobotType.PASTR);
			}


			// Check if leader is alive
			if(isLeaderAlive(rc, band)) {
				
				int command = rc.readBroadcast(band + Comm.COMMAND_SUBCHANNEL);
				if(command == Comm.HERD) {
					if(RobotPlayer.herderIndex == -1) {
						RobotPlayer.herderIndex = getHerderIndex(rc, band);
					}
					herdCows(rc, band, random, RobotPlayer.herderIndex, bugAroundEnemies);
				} else {
					RobotPlayer.herderIndex = -1;
				
				//If location is posted, go there, else, follow leader.
				int channelData;
				
				int moveToLocInt = rc.readBroadcast(band + Comm.LOCATION_SUBCHANNEL);
				if(moveToLocInt >= 0){
					channelData = moveToLocInt;
				}else{
					channelData = rc.readBroadcast(band + Comm.LEADER_LOCATION_SUBCHANNEL);
				}

				if(channelData > 10100){
					channelData -= 10100;
					MapLocation target = VectorActions.intToLoc(channelData);
					rc.setIndicatorString(1, "Moving to " + target);
					moveToLocation(rc, target, random, true, bugAroundEnemies);
				}else{
					MapLocation target = VectorActions.intToLoc(channelData);
					rc.setIndicatorString(1, "Moving to " + target);
					moveToLocation(rc, target, random, false, bugAroundEnemies);
				}
				}
			} else {
				// If leader is dead, clear leader channel and go back to idle channel
				rc.broadcast(band, 0);
				RobotPlayer.tunedChannel = Comm.IDLE_SOLDIER_CHANNEL;
			}
		}

	}

	public static void initializeLeader(RobotController rc, int band) throws GameActionException {
		broadcastVitality(rc, band);
		updateLeaderLocation(rc, band);
		RobotPlayer.friendlyPastrs = rc.sensePastrLocations(rc.getTeam());
		RobotPlayer.enemyPastrs = rc.sensePastrLocations(rc.getTeam().opponent());
		rc.setIndicatorString(1, "Current Command: " + rc.readBroadcast(band + Comm.COMMAND_SUBCHANNEL));

	}

	public static void nonCombatStrategy(RobotController rc, int band, Random random, boolean bugAroundEnemies) throws GameActionException {
		//If panicking, No SNEAKING! NO WANDERING! GO FOR THE ATTACK AND FOR BUILD.
		shouldIpanicOrRush(rc);

		if(RobotPlayer.tunedChannel == Comm.IDLE_SOLDIER_CHANNEL) {
			selectChannel(rc);

		} else if(RobotPlayer.tunedChannel == Comm.RESPONDING_SOLDIER_CHANNEL) {		
			selectBand(rc);
		} 
		else if(RobotPlayer.tunedChannel == Comm.GOOD_PASTR_CHANNEL){
			goBuildPastr(rc, random, bugAroundEnemies);
		}
		else if(RobotPlayer.tunedChannel == Comm.GOOD_NOISE_CHANNEL){
			goBuildNoiseTower(rc, random, bugAroundEnemies);
		}
		else {
			//All the defensive (And some offense with attacking enemy pastr goes here) stuff.
			defenseStrategy(rc, band, random, bugAroundEnemies);
		}	
	}

	public static boolean isFollower(RobotController rc) {
		return !RobotPlayer.isLeader && RobotPlayer.tunedChannel != Comm.GOOD_NOISE_CHANNEL && RobotPlayer.tunedChannel != Comm.GOOD_PASTR_CHANNEL;
	}

}